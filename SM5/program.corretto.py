...
def func2(parole : list[str]) -> dict[str, list[str]]:
    ...
    for parola in parole:
        iniziale = parola[0]
        if result.get(iniziale, []) == []:
            result[iniziale] = [parola]
        else:
            result[iniziale].append(parola)
    keys = result.keys()
    for key in keys:
        result[key].sort(key=lambda x: x, reverse=True)
        result[key].sort(key=lambda x: (len(x)), reverse=False)
    return result
...
L'ordinamento delle parole nei valori del risultato è per dimensioni crescenti 
...
def func3(D1 : dict[str,list[int]], D2 : dict[int,list[str]]) -> dict[str, list[str]]:
    ...
    for key in result.keys():
        result[key].sort(key=lambda w: (w))
        result[key].sort(key=lambda w: len(w))
    return result
