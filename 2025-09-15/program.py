...
def func5(file_in, file_out):
    ...
    for riga in img:
        ...
        for pixel in riga:
            if pixel == (0, 0, 0):
                continue
            if pixel in numPixel:
                numPixel[pixel] += 1
            else:
                numPixel[pixel] = 1
        
        # Handle the case where the row has no non-black pixels
        if not numPixel:
            continue
            
        maxCount = max(numPixel.values())
        coloriMax = [colore for colore, count in numPixel.items() if count == maxCount]
        coloreScelto = min(coloriMax)
        for i in range(len(riga)):
            if riga[i] != (0, 0, 0) and riga[i] != coloreScelto:
                riga[i] = (0, 0, 0)
                countPixelCambiati += 1
    images.save(img, file_out)
    return countPixelCambiati
...
def ex1(T, n):
    risultati = []
    cerca_percorsi(T, 0, [], risultati, n)
    risultati.sort()
    return risultati

def cerca_percorsi(nodo, somma_corrente, percorso_corrente, risultati, n):
        if nodo is None:
            return
        
        somma_corrente += nodo.value
        percorso_corrente.append(nodo.value)

        # Se è una foglia, controlla la somma
        if nodo.left is None and nodo.right is None:
            if somma_corrente == n:
                risultati.append(percorso_corrente.copy())
        else:
            # Continua a cercare nei sottoalberi
            cerca_percorsi(nodo.left, somma_corrente, percorso_corrente, risultati, n)
            cerca_percorsi(nodo.right, somma_corrente, percorso_corrente, risultati, n)
