...
def check_pwd(pwd: str) -> bool:
    ...
    for c in pwd:
        if c.isdigit():
            ...
            return False
    if not (has_digit and has_lower and has_upper and has_special):
        return False
    return True
...
def is_sorted(a: list) -> bool:
    order = 0  # 0 = non ordinato, 1 = crescente, -1 = decrescente
    if len(a) < 2:
        return True  # lista con un solo elemento
    if a[1] > a[0]:
            order = 1  # crescente
    elif a[1] < a[0]:
        order = -1  # decrescente
    else:
        order = 2 #uguali quindi si controlla per entrambi i casi all'inizio
    for i in range(1, len(a)):
        if order == 1 and a[i] < a[i - 1]:
...
def is_sorted_half(a: list) -> bool:
    order = 0  # 0 = non ordinato, 1 = crescente, -1 = decrescente, 2 = da controllare
    if len(a) < 3:
        return False  # lista con meno di 3 elementi
    if a[1] > a[0]:
            order = 1  # crescente
    elif a[1] < a[0]:
        order = -1  # decrescente
        ...
        if order == 1:
            if a[i] < a[i - 1]:
                if changed:
                    return False  # non è ordinato
                order = -1
                changed = True  # si suppone un cambio e lo si segna
        elif order == -1:
            if a[i] > a[i - 1]:
                if changed:
                    return False  # non è ordinato
            ...
            if a[i] > a[i - 1]:
                ...
                order = -1
    if order == 0 or not changed:
        return False  # non è ordinato
    else:
        return True  # tutti gli elementi sono uguali

# Test funzioni
check_test(check_pwd, False, "a") # type: ignore
check_test(check_pwd, False, "000000000000000000") # type: ignore
check_test(check_pwd, False, "almeno6") # type: ignore
check_test(check_pwd, False, "Aa@09asng2/") # type: ignore
check_test(check_pwd, True, "Aa@09asng2") # type: ignore
check_test(tuple_ex, (3, -2, 1), (-1, -1, 2)) # type: ignore
