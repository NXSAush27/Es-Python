...
def tribonacci(n: int):
    if n < 0:
        raise ValueError("Input non valido: n deve essere un intero non negativo.")
    if n == 0 or n == 1:
        return 0
    elif n== 2:
        return 1
...
def max_recursive(l: List[int]) -> int:
    if len(l) == 1:
        return l[0]
    if l[0] < l[1]:
        return max_recursive(l[1:])
    else:
        return max_recursive([l[0]] + l[2:])
...
def is_prime_recursive(n: int, d=2) -> bool:
    ...
    if n%d==0:
        return False
    return is_prime_recursive(n//2, d+1)
...
def knapsack(l: List[Tuple[int, int]], W: int) -> int:
    ...
    return = lista oggetti con peso complessivo = o < di W e valore totale massimizzato
    """
    n = len(l)
    if n == 0 or W == 0:
        return 0
    peso, valore = l[0]
    if peso > W:
        return knapsack(l[1:], W)
    else:
        includi_valore = valore + knapsack(l[1:], W - peso)
        escludi_valore = knapsack(l[1:], W)
        return max(includi_valore, escludi_valore)


# Test funzioni
# Memoization per ottimizzare knapsack
memo = {}

def knapsack_optimized(l: List[Tuple[int, int]], W: int, index: int = 0) -> int:
    """
    Versione ottimizzata con memoization
    """
    if index >= len(l) or W == 0:
        return 0
    
    # Chiave per memoization
    key = (index, W)
    if key in memo:
        return memo[key]
    ...
    if peso > W:
        result = knapsack_optimized(l, W, index + 1)
    else:
        includi = valore + knapsack_optimized(l, W - peso, index + 1)
        escludi = knapsack_optimized(l, W, index + 1)
        result = max(includi, escludi)
    
    memo[key] = result
    return result

check_test(knapsack_optimized, 220, [(10, 60), (20, 100), (30, 120)], 50)
check_test(knapsack_optimized, 90, [(5, 10), (4, 40), (6, 30), (3, 50)], 10)
...
check_test(knapsack_optimized, 7534, long_l, 850)
